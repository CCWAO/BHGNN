...
class ChebConv_bayes(MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, K, normalization='sym',
                 bias=True, dropout: float = 0.0, **kwargs):
        kwargs.setdefault('aggr', 'add')
        super(ChebConv_bayes, self).__init__(**kwargs)
    ...
    def forward(self, x, edge_index, edge_weight: OptTensor = None,
        ...
        if self.weight.size(0) > 1:
            ...
            Tx_1_dropout = F.dropout(torch.matmul(Tx_1, self.weight[1]), p=self.dropout, training=True)
            out = out + Tx_1_dropout

        for k in range(2, self.weight.size(0)):
            Tx_2 = self.propagate(edge_index, x=Tx_1, norm=norm, size=None)
            Tx_2 = 2. * Tx_2 - Tx_0
            Tx_2_dropout = F.dropout(torch.matmul(Tx_2, self.weight[k]), p=self.dropout, training=True)
            out = out + Tx_2_dropout
            Tx_0, Tx_1 = Tx_1, Tx_2
